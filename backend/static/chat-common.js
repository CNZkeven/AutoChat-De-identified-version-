(function () {
    function initChatPage({
        ...
        conversationApiBase = '/api/conversations',
        agent = '',
        storageKey = 'conversation_id',
        timeoutMs = 30000,
    } = {}) {
        if (!endpoint) {
            console.error('initChatPage: endpoint is required');
            return;
        }
        ...
        let messageHistory = [];
        let messagesState = [];
        let selectedRange = null; // [startIndex, endIndex]
        let isSelectMode = false;
        let currentConversationId = null;
        let conversations = [];

        const conversationBase = conversationApiBase || '/api/conversations';
        const agentParam = agent ? `agent=${encodeURIComponent(agent)}` : '';
        const listUrl = agentParam ? `${conversationBase}?${agentParam}` : conversationBase;
        const conversationUrl = (id) =>
            agentParam ? `${conversationBase}/${id}?${agentParam}` : `${conversationBase}/${id}`;
        const messagesUrl = (id) =>
            agentParam
                ? `${conversationBase}/${id}/messages?${agentParam}`
                : `${conversationBase}/${id}/messages`;
        ...
        function renderConversationList() {
            ...
            conversations.forEach((conv) => {
                ...
                meta.textContent = conv.status === 'archived' ? '已归档' : '活跃';
        ...
        async function handleRename(id) {
            ...
            try {
                ...
                renderConversationList();
                if (id === currentConversationId) setStatus('已重命名');
            } catch (e) {
                console.error(e);
        ...
        async function handleDelete(id) {
            const ok = window.confirm('确认删除该会话？删除后将无法恢复。');
            if (!ok) return;
            try {
        ...
        async function sendMessage() {
            ...
            try {
                ...
                showError(isAbort ? '请求超时，请重试' : '请求失败，请稍后重试');
        ...
        async function fetchJson(url, options = {}) {
            ...
            if (!res.ok) {
                const msg = (data && data.description) || (data && data.error) || res.statusText;
        ...
        function getSelectedMessages() {
            ...
            return messagesState.slice(Math.min(start, end), Math.max(start, end) + 1).map((m) => ({
                role: m.role,
                content: m.content,
            }));
        }
